...
export default function MyResorts(){
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-10 max-w-6xl mx-auto">
      <Link href="/owner/dashboard" className="text-sm text-resort-500 font-semibold mb-4 inline-block">← Back to Dashboard</Link>
      
      <div className="flex justify-between items-start mb-8">
        <div>
          <h1 className="text-3xl font-bold text-resort-900">My Resorts</h1>
          <p className="text-slate-600">Manage your resort listings</p>
        </div>
        <Link href="/owner/create-resort" className="px-4 py-2 bg-resort-500 text-white rounded-lg font-semibold hover:bg-resort-600 transition">
          + Create Resort
        </Link>
      </div>
      ...
      {resorts.length === 0 ? (
        <div className="bg-white border border-dashed border-slate-200 rounded-lg p-8 text-center">
          <p className="text-slate-600 mb-4">No resorts yet</p>
          <Link href="/owner/create-resort" className="text-resort-500 font-semibold">Create your first resort →</Link>
        </div>
      ) : (

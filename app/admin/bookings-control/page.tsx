...
export default function BookingsControlPage(){
  ...
  return (
    <div className="w-full px-4 sm:px-6 lg:px-8 py-10 bg-gradient-to-br from-resort-50 to-resort-100 min-h-[80vh]">
      ...
      {/* Sonner handles toast UI globally via <Toaster /> in layout */}
      ...
      <div className="max-w-6xl mx-auto">
              ...
              {confirmedBookings.map(booking => (
                <div key={booking.id} className="bg-green-50 border border-green-200 rounded-xl p-5 shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {booking.cancellation_status === 'requested' && (
                        <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded">Cancellation requested</span>
                      )}
                    </div>
                  </div>
                  ...
                  {booking.cancellation_status === 'requested' && booking.cancellation_reason && (
                    <p className="text-sm text-slate-600 mb-3">Reason: {booking.cancellation_reason}</p>
                  )}
                  <ChatLink bookingId={booking.id} as="admin" label="Open Chat" title={booking.resort?.name || undefined} />
                </div>
              ))}
        ...
        {/* Cancellation Requests (view-only for Admin; owners approve/reject) */}
        <section className="mt-8">
          <div className="flex items-center gap-2 mb-4">
            <h3 className="text-xl font-semibold text-resort-900">Cancellation Requests</h3>
            <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded font-semibold">{confirmedBookings.filter(b => b.cancellation_status === 'requested').length}</span>
          </div>
          {confirmedBookings.filter(b => b.cancellation_status === 'requested').length === 0 ? (
            <div className="bg-white border border-dashed border-slate-200 rounded-xl p-6 text-center text-slate-600">
              <p className="font-semibold text-resort-900">No pending cancellation requests</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-4">
              {confirmedBookings.filter(b => b.cancellation_status === 'requested').map(booking => (
                <div key={booking.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-semibold text-resort-900">{booking.resort?.name}</h4>
                      <p className="text-sm text-slate-600">Guest: {booking.guest?.full_name || booking.guest?.email || 'Guest'}</p>
                    </div>
                    <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded">Requested</span>
                  </div>
                  <p className="text-sm text-slate-600 mb-3">📅 {booking.date_from} → {booking.date_to}</p>
                  {booking.cancellation_reason && (
                    <p className="text-sm text-slate-600 mb-3">Reason: {booking.cancellation_reason}</p>
                  )}
                  <p className="text-xs text-slate-500">Owner must approve or reject. Admin can monitor and inform if needed.</p>
                  <div className="mt-2">
                    <ChatLink bookingId={booking.id} as="admin" label="Open Chat" title={booking.resort?.name || undefined} />
                  </div>
                </div>

-- Fix overnight booking logic to use exclusive end dates
-- This allows booking overnight 28-29 after overnight 27-28
-- because overnight 27-28 uses night of 27th, and overnight 28-29 uses night of 28th
...
BEGIN
  ...
  -- Check if guest already has ANY booking (pending or confirmed) on these dates
  -- Use EXCLUSIVE end date logic: booking 27-28 doesn't conflict with 28-29
  -- because 27-28 uses night of 27, and 28-29 uses night of 28
  SELECT EXISTS (
    SELECT 1 FROM public.bookings b
    WHERE b.guest_id = p_guest_id
      AND b.status IN ('pending', 'confirmed')
      -- Overlap if: new_start < existing_end AND new_end > existing_start (exclusive)
  ...
  -- Slot-aware overlap checking for THIS resort:
  ...
  IF v_is_single_day AND p_booking_type IN ('daytour', 'overnight') THEN
    ...
    SELECT EXISTS (
      ...
      WHERE b.resort_id = p_resort_id
        ...
        -- Use exclusive end date logic here too
        AND p_date_from < b.date_to
        AND p_date_to > b.date_from
        AND (
  ...
  ELSE
    -- Multi-day or 22hrs booking: traditional full overlap check with exclusive end
    SELECT EXISTS (
      SELECT 1 FROM public.bookings b
      WHERE b.resort_id = p_resort_id
        AND b.status = 'confirmed'
        -- Use exclusive end date logic
        AND p_date_from < b.date_to
        AND p_date_to > b.date_from
    ) INTO v_overlap;
  END IF;

-- Prevent guests from double booking across different resorts
-- A guest should not be able to book overlapping dates even if it's a different resort
...
BEGIN
  ...
  -- NEW: Check if guest already has ANY booking (pending or confirmed) on these dates
  -- across ANY resort to prevent double booking
  SELECT EXISTS (
